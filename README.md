...
1. נזהה כל טבלה כישות פוטנציאלית:
   - שם הטבלה יהפוך לשם הישות.
   - העמודות יהפכו למאפיינים.

2. נזהה את המפתח הראשי (Primary Key).

3. נזהה מפתחות זרים (Foreign Keys):
   - מהם נגזרים הקשרים (Relationships) בין הישויות.

4. נבחן שמות של טבלאות שעשויות לייצג קשרים:
   - אם טבלה מורכבת רק ממפתחות זרים לשתי טבלאות אחרות, ייתכן שהיא מייצגת קשר מסוג רבים־לרבים.

#### שלב 2: יצירת DSD

1. ניצור כל טבלה כמלבן עם שם ושדות:
   - הדגשה של המפתח הראשי.
   - סימון של מפתחות זרים.

2. נסמן קשרים בין הטבלאות על סמך המפתחות הזרים:
   - כל מפתח זר יוצר חץ מהטבלה שבה הוא נמצא לעבר הטבלה שהוא מקשר אליה.

#### שלב 3: המרה ל־ERD

1. נהפוך כל טבלה עצמאית לישות (Entity).

2. נהפוך טבלאות־קשר ל־Relationships:
   - לדוגמה: הטבלה Volunteer_Project (volunteer_id, project_id) מייצגת קשר בין Volunteer ל־Project.

3. נסמן את סוג הקשר לפי מאפייני המפתחות הזרים:
   - אם FK הוא גם PK → הקשר הוא 1:1.
   - אם FK אינו PK ואינו ייחודי → הקשר הוא 1:N או M:N.

4. נסמן השתתפות (Participation) של הישות בקשר:
   - אם ה־FK הוא not null → ההשתתפות היא חובה (Mandatory).

5. זיהוי ירושה (Generalization):
